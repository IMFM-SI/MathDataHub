...
import type { TMDHItem, TMDHProperty } from "../../client/rest"

/** A CodecExporter represents an exporter for a specific codec */
export default abstract class CodecExporter<CodecType, Accumulator = Array<CodecType>> extends ClientSideExporter<TMDHProperty, Accumulator, Blob, Array<CodecType>> {
    ...
    protected abstract open(collection: ParsedMDHCollection, property: TMDHProperty): Promise<Accumulator | null>
    ...
    // ====================
    protected initAcc(flags: TMDHProperty, collection: ParsedMDHCollection): Promise<Accumulator | null> {
        return this.open(collection, flags)
    }

    protected getPage(items: Array<TMDHItem<unknown>>, flags: TMDHProperty): Array<CodecType> {
        return items.map(item => item[flags.slug])
    }

    protected updateAcc(page: Array<CodecType>, index: number, acc: Accumulator, flags: TMDHProperty, collection: ParsedMDHCollection) {
        return this.add(acc, page, index)
    }

    protected getResult(acc: Accumulator, done: boolean, flags: TMDHProperty): Promise<Blob | null> {
