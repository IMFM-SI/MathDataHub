...
class CodecManagerTest(TestCase):
    def test_find_all_codecs(self) -> None:
        ...
        self.assertGreaterEqual(len(got), 1, msg="at least one codec is returned")

        # check that they are all codecs
        for codec in got:
            self.assertTrue(
                issubclass(codec, Codec),
                msg="CodecManager.find_all_codecs() returns only codec-subclasses",
            )

        # check that none of them are abstract
        for codec in got:
            self.assertTrue(
                not codec._meta.abstract,
    ...
    def test_find_codec(self) -> None:
        ...
        # existing codec
        self.assertEqual(
            CodecManager.find_codec("StandardInt"),
            StandardInt,
        ...
        self.assertIsNone(
            CodecManager.find_codec("mdh_data_standardint"),
        ...
        # non-existing codec
        self.assertIsNone(
            CodecManager.find_codec("This-codec-doesnt-exist"),
            msg="does not find a non-existent codec",
        )
    ...
    def test_collect_operators(self) -> None:
        ...
        self.assertSetEqual(
            CodecManager.collect_operators([StandardBool]),
            {"=", "!="},
            "Test that collecting only StandardBool operators works as expected",
        )

        self.assertSetEqual(
            CodecManager.collect_operators([StandardInt, StandardBool]),
            {"=", "<", "<=", ">", ">=", "!="},
    ...
    def test_is_valid_operand(self) -> None:
        ...
        # StandardInt
        self.assertTrue(
            StandardInt.is_valid_operand(123),
            "checks that 123 is a valid StandardInt operand",
        )
        self.assertFalse(
            StandardInt.is_valid_operand(True),
            "checks that True is not a valid StandardInt operand",
        )

        # StandardBool
        self.assertFalse(
            StandardBool.is_valid_operand(123),
            "checks that 123 is not a valid StandardBool operand",
        )
        self.assertTrue(
            StandardBool.is_valid_operand(True),
