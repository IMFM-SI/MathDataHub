...
class Z3ZCollectionTest(TestCase):
    ...
    def test_build_query(self) -> None:
        ...
        self._assert_query(
            plain_query,
            """
SELECT id, "property_value_f0_0", "property_cid_f0", "property_value_f1_0", "property_cid_f1", "property_value_f2_0", "property_cid_f2", "property_value_invertible_0", "property_cid_invertible", "property_value_label_0", "property_value_label_1", "property_cid_label" FROM (SELECT I.id as id, "T_f0".value as "property_value_f0_0", "T_f0".id as "property_cid_f0", "T_f1".value as "property_value_f1_0", "T_f1".id as "property_cid_f1", "T_f2".value as "property_value_f2_0", "T_f2".id as "property_cid_f2", "T_invertible".value as "property_value_invertible_0", "T_invertible".id as "property_cid_invertible", "T_label".label as "property_value_label_0", "T_label".params as "property_value_label_1", "T_label".id as "property_cid_label" FROM mdh_data_item as I JOIN mdh_data_itemcollectionassociation as CI ON I.id = CI.item_id AND CI.collection_id = %s LEFT OUTER JOIN mdh_data_standardint AS "T_f0" ON I.id = "T_f0".item_id AND "T_f0".active AND "T_f0".prop_id = %s LEFT OUTER JOIN mdh_data_standardint AS "T_f1" ON I.id = "T_f1".item_id AND "T_f1".active AND "T_f1".prop_id = %s LEFT OUTER JOIN mdh_data_standardint AS "T_f2" ON I.id = "T_f2".item_id AND "T_f2".active AND "T_f2".prop_id = %s LEFT OUTER JOIN mdh_data_standardbool AS "T_invertible" ON I.id = "T_invertible".item_id AND "T_invertible".active AND "T_invertible".prop_id = %s LEFT OUTER JOIN mdh_data_graphlabel AS "T_label" ON I.id = "T_label".item_id AND "T_label".active AND "T_label".prop_id = %s) AS collection
        """,
