...
from mdh_tests.models import (
    JSONArrayFieldModel,
    SmartJSONFieldModel,
    SmartNDArrayTwoModel,
    TextFieldModel,
    SmartNDArrayOneModel,
)
...
INTEGER_SAMPLES = [0, 1, -1, 123123123, -757645513]
...
JSON_SAMPLES = (
    [True, False, [1, 2, 3], [], {}, {"key": "value"}, None] + INTEGER_SAMPLES + TEXT_SAMPLES
)

JSON_ARRAY_SAMPLES = [[j, j] for j in JSON_SAMPLES]
...
class InsertionTest(TestCase):
    ...
    def test_copyfrom_integer_array(self):
        importer = CopyFromImporter(quiet=False)
        self._insert_and_compare(
            importer, SmartNDArrayOneModel, INTEGER_1D_ARRAY_SAMPLES
        )
    ...
    def test_copyfrom_integer_2darray(self):
        importer = CopyFromImporter(quiet=False)
        self._insert_and_compare(
            importer, SmartNDArrayTwoModel, INTEGER_2D_ARRAY_SAMPLES
        )
    ...
    def test_copyfrom_json_array(self):
        importer = CopyFromImporter(quiet=False)
        self._insert_and_compare(
            importer, JSONArrayFieldModel, JSON_ARRAY_SAMPLES)
