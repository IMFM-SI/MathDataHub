...
from mdh_data.tests.collection import insert_testing_data

from mdh_tests.utils import AssetPath

Z3Z_COLLECTION_PATH = AssetPath(
    __file__, "..", "..", "mdh_data", "tests", "res", "z3z_collection.json"
)
Z3Z_PROVENANCE_PATH = AssetPath(
    __file__, "..", "..", "mdh_data", "tests", "res", "z3z_provenance.json"
)
Z3Z_DATA_PATH = AssetPath(
    __file__, "..", "..", "mdh_data", "tests", "res", "z3z_data.json"
)
...
class FilterBuilderTest(TestCase):
    ...
    def test_build_filters(self) -> None:
        ...
        self.assertEqual(q1sql, '"property_value_f1_0" <= %s')
        self.assertListEqual(q1args, [1])

        # right operand
        q2sql, q2args = fb("1 <= f1")
        self.assertEqual(q2sql, '%s <= "property_value_f1_0"')
        self.assertListEqual(q2args, [1])

        # both operands
        q3sql, q3args = fb("f1 <= f2")
        self.assertEqual(q3sql, '"property_value_f1_0" <= "property_value_f2_0"')
        ...
        self.assertEqual(q6sql, 'NOT("property_value_f1_0" = %s)')
