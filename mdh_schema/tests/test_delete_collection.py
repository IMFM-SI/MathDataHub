...
class CreateCollectionTest(TestCase):
    ...
    def test_delete_collection(self) -> None:
        ...
        self.assertFalse(
            Collection.objects.exists(), "Check that there are no more collections"
        )
        self.assertFalse(
            Property.objects.exists(), "Check that there are no more properties"
        )
        self.assertFalse(
            PropertyCollectionMembership.objects.exists(),
            "Check that there are no more property-collection memberships",
        )
