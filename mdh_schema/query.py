...
from mdh_data.models import CodecManager, Codec, Item
...
class QueryBuilder(object):
    ...
    def join_builder(self) -> SQLWithParams:
        ...
        for prop in self.collection.properties():
            ...
            for i, value_name in enumerate(value_names):
                SQL += ", {}.{} as {}".format(
                    virtual_table, value_name, self._prop_value(prop, i)
                )
            SQL += ", {}.id as {}".format(virtual_table, cid_field)

        # from the item table
        SQL += " FROM {} as I".format(Item._meta.db_table)
        ...
        for prop in self.collection.properties():
            ...
            # the join
            SQL += " LEFT OUTER JOIN {} AS {}".format(physical_table, virtual_table)
    ...
    def order_builder(self, order: str, properties: Iterable[Property]) -> SQL:
        ...
        property_dict: dict[str, Property] = {p.slug: p for p in properties}
        return ", ".join(
            [self._parser_order(o, property_dict) for o in order.split(",")]
        )
...
class FilterBuilder(object):
    ...
    def _process_left(self, tree: FilterAST) -> SQLWithParams:
        ...
        prop, codec, columns = self._resolve_codec(tree["right"]["name"], op)
        lit = self._process_literal(tree["left"])
        if not codec.is_valid_operand(lit):
            raise FilterBuilderError(
                "{} is not a valid operand for codec {}".format(
                    lit, codec.get_codec_name()
                )
            )
    ...
    def _process_right(self, tree: FilterAST) -> SQLWithParams:
        op = tree["operator"]
        prop, codec, columns = self._resolve_codec(tree["left"]["name"], op)
    ...
    def _process_both(self, tree: FilterAST) -> SQLWithParams:
        ...
        if codecL is not codecR:
            raise FilterBuilderError(
                "Cannot compare properties {} and {}: Distinct codecs are not supported. ".format(
                    lvalue, rvalue
                )
            )
    ...
    def _resolve_codec(
        self, slug: str, op: str
    ) -> tuple[Property, Type[Codec], List[str]]:
        ...
        if op not in codec.operators:
            raise FilterBuilderError(
                "Codec {} does not support operator {}".format(
        ...
        columns = [
            QueryBuilder._prop_value(p, i) for i in range(len(codec.get_value_fields()))
        ]
    ...
    def _process_unary(self, tree: FilterAST) -> SQLWithParams:
        ...
        op = tree["operator"]
        if op != "!":
            raise FilterBuilderError("Unknown unary operator {} found".format(op))
    ...
    def _raise_error_for_type(self, tp: str):
        ...
        if tp == "Compound":
            ...
            raise FilterBuilderError("Unknown Expression Type: Something went wrong. ")
