...
from mdh.utils import ModelWithMetadata, QuerySetLike
...
class Collection(ModelWithMetadata):
    ...
    slug: str = models.SlugField(help_text="Identifier of this collection", unique=True)
    ...
    url: Optional[str] = models.URLField(
        null=True,
        blank=True,
        help_text="URL for more information about this collection",
    )
    ...
    viewName: Optional[str] = models.SlugField(
        help_text="Name for the (potentially materialized) view of this collection (if any)",
        unique=True,
        null=True,
        blank=True,
    )
    ...
    template: Optional[str] = models.TextField(
        help_text="Custom template for rendering this collection",
        null=True,
        blank=True,
        default=None,
    )

    exporters = models.ManyToManyField(
        "Exporter",
        help_text="List of enabled exporters for this collection",
        blank=True,
    )
    ...
    flag_large_collection: bool = models.BooleanField(
        default=False,
        help_text="Flag this collection as potentially large to the user interface",
    )
    ...
    def view(self) -> Optional[View]:
        ...
        return View.make_view(
            ...
            mview_sql,
            mview_sql_params,
            materialized=View.supports_materialization(connection),
        )
...
def collection_save(
    sender: Type[Collection], instance: Collection, **kwargs: Any
) -> None:
...
class Property(ModelWithMetadata):
    ...
    default: bool = models.BooleanField(
        default=True, help_text="Show property by default in frontend interface"
    )
    ...
    collections = models.ManyToManyField(
        Collection,
        through="PropertyCollectionMembership",
        help_text="Collection(s) this property occurs in",
        blank=True,
    )
