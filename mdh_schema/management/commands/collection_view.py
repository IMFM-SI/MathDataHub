...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            "-s",
            action="store_true",
            help="Syncronizes the views after creation",
        )
    ...
    def handle(self, *args: Any, **kwargs: Any) -> None:
        with connection.cursor() as cursor:
            logger = logging.getLogger("mdh.collection")
    ...
    def handle_enable(
        ...
        collection.viewName = "mdh_view_{}".format(collection.slug)
        collection.view  # this line is required to ensure that the mviews app can find it
        collection.save()
        logger.info(
            "Adding view {} to collection {}".format(
                collection.viewName, collection.slug
            )
        )
    ...
    def handle_disable(
        ...
        if sync and view is not None:
            logger.info(
                "Removing view {} from collection {}".format(view.name, collection.slug)
            )
