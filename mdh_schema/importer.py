...
from mdh_schema.models import Collection, Exporter, Property, PreFilter
...
class SchemaImporter(object):
    ...
    def __init__(self, data: Any = None, root_path: str = "", quiet: bool = False):
        self.logger = logging.getLogger("mdh.schemaimporter")
    ...
    def _validate_data(self, data: Any) -> None:
        ...
        if "hidden" in data and not isinstance(data["hidden"], bool):
            raise SchemaValidationError("Key {0!r} is not a boolean. ".format("hidden"))
        ...
        if "exporters" in data:
            if not isinstance(data["exporters"], list):
                raise SchemaValidationError("Key 'exporters' is not a list. ")
    ...
    def _validate_property(self, data: Any) -> None:
        ...
        for k in ["description", "url"]:
            if k in data and not isinstance(data[k], (str, type(None))):
                raise SchemaValidationError(
                    "Property {0!r}, Key {1} is not a string or undefined. ".format(
                        slug, k
                    )
                )
    ...
    def _validate_pre_filter(self, data: Any) -> None:
        for k in ["name", "condition"]:
            if k not in data:
                raise SchemaValidationError("Prefilter missing key {}. ".format(k))
            if not isinstance(data[k], str):
                raise SchemaValidationError(
    ...
    def __call__(self, update: bool = False) -> tuple[Collection, bool]:
        ...
        self.logger.info(
            "{1!s} collection {0!r}".format(slug, "Created" if created else "Updated")
        )
    ...
    def __call__property(
        self, collection: Collection, prop: dict[str, str], update: bool = False
    ) -> tuple[Property, bool]:
        """
        ...
        prop_set: Optional[QuerySet[Property]] = collection.property_set.filter(
            slug=slug
        )
        if prop_set:
            if not update:
            ...
            self.logger.info("Updating property {0!r} (already exists). ".format(slug))
